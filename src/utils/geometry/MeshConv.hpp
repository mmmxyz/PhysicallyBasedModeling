	...
	uint32_t & outISize)
{
	...
	outVertSize = Rvsize;
	*pOutVdata = new fvec3[Rvsize];

	outISize = Risize;
	*pOutIData = new uint32_t[Risize];

	for (uint32_t i = 0; i < Rvsize; i++) {
		(*pOutVdata)[i].x = Pvdata[i].x;
		(*pOutVdata)[i].y = Pvdata[i].y;
		(*pOutVdata)[i].z = Pvdata[i].z;

		(*pOutIData)[i] = Ridata[i];
...
void ComputeNormalFromVE(
	const uint32_t vertsize,
	const fvec3* const PositionList,
	fvec3* const NormalSet,
	const uint32_t* const tilist,
