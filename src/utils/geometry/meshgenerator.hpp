    ...
    const fvec3& bias)
{
	...
	for (uint32_t i = 0; i < InnerEdgesize / 4; i++) {
		...
		float angle210 = std::acos(((X2 - X1).normalized()).dot((X0 - X1).normalized()));
		float angle201 = std::acos(((X2 - X0).normalized()).dot((X1 - X0).normalized()));
		float angle310 = std::acos(((X3 - X1).normalized()).dot((X0 - X1).normalized()));
		float angle301 = std::acos(((X3 - X0).normalized()).dot((X1 - X0).normalized()));
